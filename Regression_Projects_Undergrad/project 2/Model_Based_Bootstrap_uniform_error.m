% Model Based Bootstrap Regression Analysis with uniform error
...
var_r = zeros(n,1);
for i=1:n
    var_r(i) = stats(4)*(1-H(i,i));
end
rs = r./var_r;  % Revised Studentized Residuals
rs_r = rs - mean(rs);   % Centralized Residuals
f = @(x) x;
bootstat = bootstrp(B,f,rs_r);
...
for j=1:B
    Y_b(:,j) = b_OLS(1) + b_OLS(2)*X_2 + (bootstat(j,:))';
    [b_OLS_b(:,j),bint_b(:,:,j),r_b(:,j),rint_b(:,:,j),stats_b(:,j)] = regress(Y_b(:,j),X);
...
CI_L_beta1_OLS_b = b_OLS_b(2,:)-((omega_OLS*sigma_cap_OLS_b)./(sqrt(sum((X_2-mean(X_2)).^2))));
CI_U_beta1_OLS_b = b_OLS_b(2,:)+((omega_OLS*sigma_cap_OLS_b)./(sqrt(sum((X_2-mean(X_2)).^2))));
...
for k = 1:B
    for i = 1:n
        for j = 1:n
            if i~=j
                beta1_LAD_b(i,j,k) = (Y_b(j,k)-Y_b(i,k))/(X_2(j)-X_2(i));
                beta0_LAD_b(i,j,k) = Y_b(j,k)-beta1_LAD_b(i,j,k)*X_2(j);
                d(i,j,k) = sum(abs(Y_b(:,k)-(beta0_LAD_b(i,j,k)+beta1_LAD_b(i,j,k)*X_2)));
            end
        end
    end
end
...
for j=1:B
    sigma_cap_LAD_b(j) = sqrt((sum((Y_b(:,j)-(b0_LAD+b1_LAD*X_2)).^2))/(n-2));
    theta_star_LAD_b(j) = (b1_LAD_b(j)-(b1_LAD))/((sigma_cap_LAD_b(j))/(sqrt(sum((X_2-mean(X_2)).^2))));
end
...
% In Bootstrap samples
CI_L_beta1_LAD_b = b1_LAD_b(1,:)-((omega_LAD*sigma_cap_LAD_b)./(sqrt(sum((X_2-mean(X_2)).^2))));
CI_U_beta1_LAD_b = b1_LAD_b(1,:)+((omega_LAD*sigma_cap_LAD_b)./(sqrt(sum((X_2-mean(X_2)).^2))));
...
for l = 1:B
    for i = 1:n
        for j = 1:n
            for k=1:n
                beta1_LMS_b(i,j,k,l) = (Y_b(i,l)-Y_b(k,l))/(X_2(i)-X_2(k));
                beta0_LMS_b(i,j,k,l) = Y_b(j,l)+Y_b(k,l)-beta1_LMS_b(i,j,k,l)*(X_2(j)+X_2(k));
                d(i,j,k,l) = median((Y_b(:,l)-(beta0_LMS_b(i,j,k,l)+beta1_LMS_b(i,j,k,l)*X_2)).^2);
            end
        end
    end
end
...
for j=1:B
    sigma_cap_LMS_b(j) = sqrt((sum((Y_b(:,j)-(b0_LMS+b1_LMS*X_2)).^2))/(n-2));
    theta_star_LMS_b(j) = (b1_LMS_b(j)-(b1_LMS))/((sigma_cap_LMS_b(j))/(sqrt(sum((X_2-mean(X_2)).^2))));
end
...
% In Bootstrap samples
CI_L_beta1_LMS_b = b1_LMS_b(1,:)-((omega_LMS*sigma_cap_LMS_b)./(sqrt(sum((X_2-mean(X_2)).^2))));
CI_U_beta1_LMS_b = b1_LMS_b(1,:)+((omega_LMS*sigma_cap_LMS_b)./(sqrt(sum((X_2-mean(X_2)).^2))));
